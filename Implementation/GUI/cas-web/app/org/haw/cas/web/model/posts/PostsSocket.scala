...
object PostsSocket extends LowLog{

  lazy implicit val timeout = Timeout(100 second)
  lazy val system = Environment.system
  lazy val actor = system.actorOf(Props[PostsSocketActor], "postssocketbuffer")
  
  def apply() = {
    (actor ? Messages.updatePosts).map {
      case Connect(out) => {
