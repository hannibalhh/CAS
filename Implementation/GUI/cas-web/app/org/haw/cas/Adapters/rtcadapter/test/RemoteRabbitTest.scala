...
object RemoteRabbitTest extends App{
 
  val config = ConfigFactory.load()
  val system = ActorSystem("myrabbit",config.getConfig("rabbit").withFallback(config))
  
  val sender = system.actorOf(Props[Sender],"send")
  val consumer = system.actorOf(Props[Sender],"consumer")
  
  val system2 = ActorSystem("foo",config.getConfig("needs").withFallback(config))

  val foo = system2.actorOf(Props[FooRemote],"foo")
